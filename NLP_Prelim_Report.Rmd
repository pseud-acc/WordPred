...
The processed corpus was then tokenised into a document frequency matrix (dfm) containing n-grams for n=1,2,3,4. Separate frequency tables were then created for the complete corpus and the individual source texts. To reduce the size of the corpus and increase the speed of the prediction algorithms, only n-grams with a frequency of 3 or more were retained. 
...
for (i in c("news","blogs","twitter")){
       wc_uni[[i]] <- wordcloud_obj(output_uni$freq.table_by_group,nwords,i,0.9,0.4,480)
       wc_bi[[i]] <- wordcloud_obj(output_bi$freq.table_by_group,nwords,i,0.6,0.4,480)
       wc_tri[[i]] <- wordcloud_obj(output_tri$freq.table_by_group,nwords,i,0.8,0.4,800)
       wc_quad[[i]] <- wordcloud_obj(output_quad$freq.table_by_group,nwords,i,0.4,0.4,480)
}

wc_uni[["All"]] <- wordcloud_obj(output_uni$freq.table,nwords,"All",0.9,0.4,480)
wc_bi[["All"]] <- wordcloud_obj(output_bi$freq.table,nwords,"All",0.6,0.4,480)
wc_tri[["All"]] <- wordcloud_obj(output_tri$freq.table,nwords,"All",0.8,0.4,800)
wc_quad[["All"]] <- wordcloud_obj(output_quad$freq.table,nwords,"All",0.4,0.4,480)
```
...
**Frequency Plots**
```{r bigram_freq_plot, include=TRUE, echo = FALSE, eval = TRUE, cache = FALSE, fig.align='center', fig.height=5, fig.width=12, fig.cap="**Figure 4**: Bi-gram Frequency Plots; (left) unigram frequencies for top 100 most frequent bi-grams, (right) cumulative frequency density up to 20th percentile of bi-grams."}
